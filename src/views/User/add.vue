			...
			<div class="align-center" style="width: 100%;">
				<el-form ref="form" :model="form" :rules="rules" label-width="35%" label-position="right">
					<el-form-item label="用户名：" prop="username">
						<el-input placeholder="必填，字母与数字组合，不超过10位字符" maxlength="10" v-model="form.username">
						</el-input>
					</el-form-item>
					<el-form-item label="姓名：" prop="full_name">
						<el-input placeholder="必填，员工姓名，不超过10位字符" maxlength="10" v-model="form.full_name">
						</el-input>
					</el-form-item>
					...
					<el-form-item label="组织结构：" prop="org_id">
						<el-tree style="width:500px" :data="treeData" :props="defaultProps" :highlight-current="true" @node-click="handleNodeClick">
						</el-tree>
					</el-form-item>
					<el-form-item label="选择角色：" prop="role_keys">
						<el-select multiple v-model="form.role_keys" placeholder="请选择角色">
							<el-option v-for="item in roles" :key="item.role_key" :label="item.role_name" :value="item.role_key">
							</el-option>
						</el-select>
					</el-form-item>
					<el-form-item label="密码：" prop="yun_password">
						<el-input type="password" placeholder="8-18位，需包含字母、数字" maxlength="18" v-model="form.yun_password">
						</el-input>
					</el-form-item>
					<el-form-item label="确认密码：" prop="confirmPass">
						<el-input type="password" placeholder="8-18位，需包含字母、数字" maxlength="18" v-model="form.confirmPass">
						</el-input>
					</el-form-item>
					<el-form-item label="备注：" prop="remark">
						<el-input placeholder="非必填，不超过40字符" maxlength="40" v-model="form.remark">
						</el-input>
					</el-form-item>
					<div class="cls"></div>
					<div class="cls"></div>
	...
	export default {
		...
		data() {
			...
			return {
				...
				rules: {
					username: [{
							message: "必填，字母与数字组合，不超过10位字符",
							required: true,
							trigger: 'blur'
						},
						{
							message: "必填，字母与数字组合，不超过10位字符",
							pattern: ptn.en_num(1, 10),
							trigger: 'blur'
						}
					],
					full_name: [{
							message: "必填，员工姓名，不超过10位字符",
							required: true,
							trigger: 'blur'
						},
						{
							message: "必填，员工姓名，不超过10位字符",
							pattern: ptn.cn_en_num_cen_line(1, 10),
							trigger: 'blur'
						}
					],
					org_id: [{
					...
					yun_password: [{
							message: "8-18位，需包含字母、数字",
							required: true,
							trigger: 'blur'
						},
						{
							message: "8-18位，需包含字母、数字",
							validator: ptn.validatePassword,
							trigger: 'blur'
						}
					],
					confirmPass: [{
		...
		methods: {
			...
			submit() {
				...
				this.$refs.form.validate((valid) => {
					if(valid) {
						that.$post("/user/edit", {
							UserRequest: that.form
						}).then(response => {
							if(response.code == 0) {
								that.$message.error(response.msg);
								return;
							}
							that.$util.successAlert("用户新增成功！");
						})
					} else {
